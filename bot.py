...
DIAS_FILTRO_NOTICIAS = 3  # Considerar notícias dos últimos 3 dias (AMPLIADO: era 1 dia)
...
async def buscar_noticias_google_news(palavras_chave, max_resultados=10):
    ...
    try:
        logger.info(f"🔍 Buscando notícias no Google News para: {palavras_chave[:3]}...")
        ...
        logger.error(f"Erro ao buscar no Google News: {e}")
        return []
...
class MonitoramentoManager:
    ...
    async def executar_monitoramento(self, executar_imediatamente=False):
        ...
        # PRIORIDADE 1: GOOGLE NEWS (SEMPRE ATIVO)
        try:
            logger.info("🌟 FONTE PRINCIPAL: Google News")
            ...
            logger.info(f"📊 Google News: {len(noticias_google)} notícias encontradas, {len([n for n in noticias_google if not self.ja_foi_notificado(n['url'], config)])} novas")
        ...
        logger.info(f"📊 Monitoramento concluído: Google News + {total_links_descobertos} links de sites, {len(resultados)} alertas gerados")
